...
*/
SELECT COUNT(gen.cus_gender)AS NoOfCustomers,gen.cus_gender FROM
(SELECT cus.cus_id,cus.cus_gender,cus.ord_amount,cus.cus_name FROM
(SELECT `order`.ord_amount,`order`.cus_id,customer.cus_gender,customer.cus_name
FROM `order`
INNER JOIN
customer
ON `order`.cus_id=customer.cus_id
HAVING `order`.ord_amount>=3000)AS cus
GROUP BY cus.cus_id)AS gen
GROUP BY gen.cus_gender;
/*
...
/*
Find the least expensive product from each category and print the table with category
 id, name, product name and price of the product
*/
